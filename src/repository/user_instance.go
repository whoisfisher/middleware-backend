...
func (u userInstanceRepository) Get(userID string) ([]model.UserInstance, error) {
	...
	if err := db.DB.Where("user_id = ?", userID).Find(&userInstances).Error; err != nil {
		if gorm.ErrRecordNotFound == err {
			logger.Log.Errorf("Failed to get instance info by userid[%s]: %s", userID, err.Error())
			return nil, nil
		}
...
func (u userInstanceRepository) Update(ID string, values map[string]interface{}) error {
	err := db.DB.Model(model.UserInstance{}).Where("id = ?", ID).Updates(values).Error
	if err != nil {
		logger.Log.Errorf("Failed to update user-instance info: %s", err.Error())
		return err
	}
	return nil
}
