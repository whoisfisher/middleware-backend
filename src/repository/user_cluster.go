...
func (u userClusterRepository) Get(userID string) ([]model.UserCluster, error) {
	...
	if err := db.DB.Where("user_id = ?", userID).First(&userClusters).Error; err != nil {
		logger.Log.Errorf("Failed to get cluster info by userid[%s]: %s", userID, err.Error())
...
func (u userClusterRepository) Update(ID string, values map[string]interface{}) error {
	err := db.DB.Model(model.UserCluster{}).Where("id = ?", ID).Updates(values).Error
	if err != nil {
		logger.Log.Errorf("Failed to update user-cluster info: %s", err.Error())
		return err
	}
	return nil
}
