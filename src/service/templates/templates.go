...
func (t templatesService) Get(name string) (*entity.Templates, error) {
	...
	if err != nil {
		logger.Log.Errorf("Faile to get template info by name[%s]: %s", name, err.Error())
		return nil, err
	}
...
func (t templatesService) GetById(id string) (*entity.Templates, error) {
	var template entity.Templates
	modelTemplate, err := t.templateRepo.GetById(id)
	if err != nil {
		logger.Log.Errorf("Faile to get template info by id[%s]: %s", id, err.Error())
		return nil, err
	}
	template.Templates = modelTemplate
	return &template, nil
...
func (t templatesService) Page(num, size int) (*entity.Page, error) {
	...
	if err != nil {
		logger.Log.Errorf("Faile to get template list info: %s", err.Error())
		return nil, err
	}
	page := entity.Page{}
	page.Total = total
...
func (t templatesService) Save(template entity.Templates) error {
	...
	if err != nil {
		logger.Log.Errorf("Faile to get template info by name[%s]: %s", template.Name, err.Error())
		return err
	}
	...
	if err != nil {
		logger.Log.Errorf("Faile to save template info to database: %s", err.Error())
		return err
	}
	return nil
}
...
func (t templatesService) Update(template entity.Templates) error {
	...
	if err != nil {
		logger.Log.Errorf("Faile to marshal template info: %s", err.Error())
		return err
	}
	...
	if err != nil {
		logger.Log.Errorf("Faile to unmarshal template info: %s", err.Error())
		return err
	}
