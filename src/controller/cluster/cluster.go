...
func GetClusterByUserId(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Get cluster failed: %s", err.Error())
		ginx.Dangerous(err)
	}
...
func ImportCluster(ctx *gin.Context) {
	...
	if err := ctx.ShouldBind(&clusterDTO); err != nil {
		logger.Log.Errorf("ClusterInfo bind failed: %s", err.Error())
		ginx.Dangerous(err)
	}
	err := clusterController.ClusterService.Import(clusterDTO)
	if err != nil {
		logger.Log.Errorf("Import Cluster failed: %s", err.Error())
		ginx.Dangerous(err)
	}
...
func DeleteCluster(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Delete Cluster failed: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(nil, nil)
}
