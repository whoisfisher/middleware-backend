...
func Get(ctx *gin.Context) {
	instanceName := ctx.Query("postgres_name")
...
func Create(ctx *gin.Context) {
	...
	if err := ctx.ShouldBind(&postgres); err != nil {
		logger.Log.Errorf("Postgres info bind failed: %s", err.Error())
		ginx.Dangerous(err)
	}
	instanceId, err := postgresController.PostgresService.Create(postgres)
	if err != nil {
		logger.Log.Errorf("Failed to create postgres instance from template: %s", err.Error())
		ginx.Dangerous(err)
	}
...
func Delete(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to delete postgres instance: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(nil, nil)
}
...
func Update(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to update postgres instance: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(nil, nil)
}
...
func Edit(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to edit postgres instance: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(nil, nil)
}
...
func GetPodLogs(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Create websocket channel failed: %s", err.Error())
		ws.WriteMessage(websocket.TextMessage, []byte(err.Error()))
	}
	err = ws.ReadJSON(&postgresLog)
	if err != nil {
		logger.Log.Errorf("Failed to read postgres info: %s", err.Error())
		ws.WriteMessage(websocket.TextMessage, []byte(err.Error()))
	}
	log := make(chan string)
	postgresController.PostgresService.GetPostgresLog(postgresLog.Instance, log)
	...
	for {
		...
		if ok := strings.Contains(data, cn); ok {
			...
			if err != nil {
				logger.Log.Errorf("Failed to write websocket info: %s", err.Error())
				ws.WriteMessage(websocket.TextMessage, []byte(err.Error()))
			}
...
func GetStatus(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to get postgres status: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(instanceObj, nil)
}

// 获取postgres operator名称
// @Tags 获取postgres operator名称
// @Summary: 获取postgres operator名称
// @Description: 获取postgres operator名称
// @Accept json
// @Param Authorization	header string true "Bearer 31a165baebe6dec616b1f8f3207b4273"
// @Param   postgres_id     query    string     true        "postgresql实例ID"
// @Success 200
// @Router /api/v1/postgres/operator [get]
func GetPostgresOperatorName(ctx *gin.Context) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to get postgres operator name: %s", err.Error())
		ginx.Dangerous(err)
	}
...
// 获取postgres实例的secrets
// @Tags 获取postgres实例的secrets
// @Summary: 获取postgres实例的secrets
// @Description: 获取postgres实例的secrets
// @Accept json
// @Param Authorization	header string true "Bearer 31a165baebe6dec616b1f8f3207b4273"
// @Param   postgres_id     query    string     true        "postgresql实例ID"
// @Success 200
// @Router /api/v1/postgres/secret/list [get]
func GetPostgresSecrets(ctx *gin.Context) {
	clusterId := ctx.Query("cluster_id")
	name := ctx.Query("name")
	...
	secretInfos, err := postgresController.PostgresService.GetUsernameAndPassword(instance)
	if err != nil {
		logger.Log.Errorf("Failed to get postgres instance secrets: %s", err.Error())
		ginx.Dangerous(err)
	}
	ginx.NewRender(ctx).Data(secretInfos, nil)
}
