...
import (
	...
	helm "github.com/mensylisir/kmpp-middleware/src/controller/helm"
	"github.com/mensylisir/kmpp-middleware/src/controller/instance"
	"github.com/mensylisir/kmpp-middleware/src/controller/kubesphere"
	"github.com/mensylisir/kmpp-middleware/src/controller/namespace"
...
func configHttpRouter(rg *gin.RouterGroup, version string) {
	...
	rg.GET("/postgres/list", postgres.Page)
	...
	rg.GET("/postgres/secret/list", postgres.GetPostgresSecrets)
	rg.GET("/postgres/operator", postgres.GetPostgresOperatorName)

	rg.PATCH("/svc", svc.UpdateServiceType)

	rg.POST("/namespace", namespace.Create)

	//helm repository
	rg.POST("/helm/repository", helm.Add)
	rg.PATCH("/helm/repository", helm.Update)
	rg.DELETE("/helm/repository", helm.Delete)

	rg.POST("/cluster/helm/repository", helm.AddToCluster)
	rg.POST("/cluster/helm/application", helm.InstallChart)
	rg.POST("/cluster/helm/application/list", helm.ListDeployChart)
	rg.DELETE("/cluster/helm/application", helm.UninstallChart)

	// iam
	rg.GET("/cluster/iam/user/list", kubesphere.ListUsers)

	//tenant
	rg.GET("/cluster/tenant/workspace/list", kubesphere.ListWorkspaces)

	//project
	rg.GET("/cluster/workspace/project/list", kubesphere.ListProjects)

	rg.GET("/cluster/iam/globalrolebinding/list", kubesphere.ListGlobalRoleBindings)
	rg.GET("/cluster/iam/globalrolebinding/permission", kubesphere.GetGlobalPermisson)

	rg.GET("/cluster/iam/workspacerolebinding/list", kubesphere.ListWorkspaceRoleBindings)
	rg.GET("/cluster/iam/workspacerolebinding/permission", kubesphere.GetWorkspacePermisson)

	rg.GET("/cluster/iam/rolebinding/list", kubesphere.ListRoleBindings)
	rg.GET("/cluster/iam/rolebinding/permission", kubesphere.GetPermisson)
}
